...
| Situation | Ask about |
|-----------|-----------|
| "Refactor this module" | Approach: extract functions vs. split files vs. rename only |
| "Add tests" | Scope: unit only, integration only, or both |
| "Clean up the footer" | Change: layout only, colors only, or full redesign |
| "Update the config" | Target: project `.pi/settings.json` or global `~/.pi/agent/settings.json` |
